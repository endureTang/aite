...
<link rel="stylesheet" href="static/css/order/order_list.css">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="statistics clearfix">
                            <p class="pull-right">审批金额统计: <span id="recordsAuditAmount">0</span></p>
                            <p class="pull-right">申请金额统计: <span id="moneyTotal">0</span></p>
                            <p class="pull-right">订单统计: <span id="recordsTotal">0</span>件</p>
                        </div>
        ...
        renderTable: function () {
            ...
            this.defTable = $('#default_table').DataTable({
                ...
                ajax : function (data, callback, settings) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: util.getServerUrl('/loanOrder/listOrderPage'),
                        ...
                        success : function (result) {
                            ...
                            returnData.recordsAmount = result.recordsAmount;//返回数据全部记录
                            returnData.recordsFiltered = result.recordsFiltered;//后台不实现过滤功能，每次查询均视作全部结果
                            returnData.data = result.data;//返回的数据列表

                            _this.totalData.recordsAmount = result.recordsAmount;
                            _this.totalData.recordsTotal = result.recordsTotal;
                            _this.totalData.recordsAuditAmount = result.recordsAuditAmount;
                ...
                columns: [
                    {"data": "orderId"},
                    {
                        ...
                        "render": function (data, type, row,meta) {
                            return '<a data-model="navTab" data-url="<%=basePath%>loanOrder/baseInfoPage?start='+ meta.settings.oAjaxData.start +'&keyword='+$.trim($('#search').val())+'&orderStatus='+$.trim($('#orderStatus').val())+'&period='+$.trim($('#period').val())+'&createDate='+$.trim($('#beginTime').val())+'&totalOrder=1&orderId=' + row.orderId + '" class="order-num">' + row.orderNo + '</a>'
                        }
                    },
                    {
                        "data": null,
                        "render" : function (data, type, row) {
                            var arr = ['平台拒绝','用户拒绝','初审驳回'];
                    ...
                    { // 终审金额
                        "data": null,
                        "render": function (data, type, row) {
                            return row.auditAmount == '' ? '-' : row.auditAmount
                        }
                    },
                    { // 终审期限
                        "data": null,
                        "render": function (data, type, row) {
                            return row.auditPeriod == '' ? '-' : row.auditPeriod
                        }
                    },
                    { // 利率
                        "data": null,
                ...
                initComplete: function (setting, json) {
                    // 渲染订单和总金额
                    $('#moneyTotal').text(_this.formatMoney(_this.totalData.recordsAmount,2));
                    $('#recordsAuditAmount').text(_this.formatMoney(_this.totalData.recordsAuditAmount,2));
                    $('#recordsTotal').text(_this.totalData.recordsTotal);
                },
                drawCallback : function (setting) {
                    // var json = _this.defTable.ajax.params();
                    // 表格重绘后再次渲染订单和总金额
                    $('#moneyTotal').text(_this.formatMoney(_this.totalData.recordsAmount,2));
                    $('#recordsAuditAmount').text(_this.formatMoney(_this.totalData.recordsAuditAmount,2));
                    $('#recordsTotal').text(_this.totalData.recordsTotal);
