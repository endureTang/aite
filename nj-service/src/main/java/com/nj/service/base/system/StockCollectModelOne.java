...
@Transactional
@Service
public class StockCollectModelOne {
    /**
     ...
     */
    public List<StockCollect> execExcel(File file, String type) throws Exception {
        ...
        if(!ExcelUtil.EMPTY.equals(postfix)){
            if(ExcelUtil.OFFICE_EXCEL_2003_POSTFIX.equals(postfix)){
                ...
                if(titleRow != null){
                    //读取Row,从标题行的下一行开始
                    List<StockCollect> stockCollectList = new ArrayList<>();
                    for(int rowNum = titleRow+1;rowNum <= totalRows;rowNum++){
                        ...
                        if(hssfRow!=null){
                            ...
                            if(cell!=null){
                                ...
                                stockCollect.setId(UuidUtil.get32UUID());
                                stockCollect.setDepartment(department);
                                stockCollect.setMeterialNo(meterialNo);
                                stockCollect.setDescription(description);
                                stockCollect.setBradCode(brandCode);
                                stockCollect.setBrand(brand);
                                stockCollect.setPubDate(publishDate);
                                stockCollect.setSexType(sexType);
                                stockCollect.setSportType(sportType);
                                stockCollect.setSeasonCode(seasonCOde);
                                stockCollect.setColorAll(colorAll);
                                stockCollect.setSize(size);
                                stockCollect.setFirstReceiveDate(firstReceiveDate);
                                stockCollect.setCostPrice(costPrice);
                                stockCollect.setRrp(rrp);
                                stockCollect.setCrp(crp);
                                stockCollect.setErp(epr);
                                stockCollect.setAmount(amount);
                                stockCollect.setOnWayAmount(onTheWayAmount);
                                stockCollect.setStorage(storage);
                                stockCollect.setStoreName(fileName.substring(0,fileName.lastIndexOf(".")));
                                stockCollect.setType(type);
                ...
                if(titleRow != null){
                    ...
                    for(int rowNum = titleRow+1;rowNum <= totalRows;rowNum++){
                        ...
                        if(xssfRow!=null){
                            ...
                            String amount = ExcelUtil.getXValue(xssfRow.getCell(16));
                            String onTheWayAmount = ExcelUtil.getXValue(xssfRow.getCell(17));
                            String storage = ExcelUtil.getXValue(xssfRow.getCell(18));
                            StockCollect stockCollect = new StockCollect();
                            stockCollect.setId(UuidUtil.get32UUID());
                            stockCollect.setDepartment(department);
                            stockCollect.setMeterialNo(meterialNo);
