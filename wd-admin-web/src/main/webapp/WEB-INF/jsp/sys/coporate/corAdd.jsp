...
+ path + "/";
%>
<form class="form-horizontal" action="coporate/corAdd" method="post"
    ...
    <div class="modal-header">
        <div class='bootstrap-dialog-header'>
            ...
            <div class='bootstrap-dialog-title'>新增企业</div>
        </div>
    </div>
    <div class="modal-body">
        <div class="container-fluid">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <input id="companCode" name="companCode" type="text" maxlength="32"
                           minlength="2" class="form-control required" placeholder="请输入企业编号">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <input id="companName" name="companName" type="text" maxlength="32"
                           minlength="2" class="form-control required" placeholder="请输入企业名称">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <input id="address" name="address" type="text" maxlength="32"
                           minlength="2" class="form-control required" placeholder="请输入企业地址">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <input id="maxCreditLimit" name="maxCreditLimit" type="number" maxlength="6"
                           minlength="1" class="form-control required" placeholder="请输入最大授信额度">
                </div>
            </div>
                ...
                <div class="col-sm-7">
                    <c:forEach var="payChannelMap" items="${payChannelList}">
                        <c:forEach var="item" items="${payChannelMap}">
                            <label><input name="payWay" class="item" type="radio" value="${item.key}">${item.value}</label>
                        </c:forEach>
                    </c:forEach>
                </div>
            </div>
                ...
                <div class="col-sm-7">
                    <c:forEach var="capticalEndListMap" items="${capticalEndList}">
                        <c:forEach var="item" items="${capticalEndListMap}">
                            <label><input name="pushWay" class="item" type="radio" value="${item.key}">${item.value}</label>
                        </c:forEach>
                    </c:forEach>
                </div>
            </div>
                ...
                <div class="col-sm-7">
                    <c:forEach var="personTemplateListMap" items="${personTemplateList}">
                        <c:forEach var="item" items="${personTemplateListMap}">
                            <label><input class="item productModel" type="checkbox" value="${item.key}">${item.value}</label>
                        </c:forEach>
                    </c:forEach>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#saveEdit').click(function () {
            ...
            // 判断有无小数点
            if( /^[0-9]\d{0,5}$/.test(Number($('#maxCreditLimit').val())) && String($('#maxCreditLimit').val()).indexOf('.')==-1){
                $('#defForm').submit();
            }else{
