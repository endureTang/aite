            ...
            renderTable: function () {
                ...
                this.defTable = $('#default_table').DataTable({
                    ...
                    ajax : function (data, callback, settings) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: util.getServerUrl('/loanOrder/myLoanOrderList'),
                            ...
                            success : function (result) {
                                if(result.status == 0) {
                                    ...
                                    _this.alertModelCancel(result.msg);
                                }
                            },
                            error : function (error) {
                                _this.alertModelCancel(error);
                            }
                        })
                    },
                    language: {
                    ...
                    columns: [
                        {"data": "orderId"},
                        {
                            ...
                            "render": function (data, type, row,meta) {
                                return '<a data-model="navTab" data-url="<%=basePath%>loanOrder/baseInfoPage?totalOrder=0&start='+ meta.settings.oAjaxData.start +'&keyword='+$.trim($('#search').val())+'&orderStatus='+$.trim($('#orderStatus').val())+'&period='+$.trim($('#period').val())+'&beginTime='+$.trim($('#beginTime').val())+'&endTime='+$.trim($('#endTime').val())+'&orderId=' + row.orderId + '" class="order-num">' + row.orderNo + '</a>'
                            }
                        },
